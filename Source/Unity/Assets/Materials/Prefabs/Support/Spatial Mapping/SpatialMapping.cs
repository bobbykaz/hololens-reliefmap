...
public class SpatialMapping : MonoBehaviour
{
    ...
    public void SetMaterialTarget(Vector3 target)
    {
        DrawMaterial.SetVector("", new Vector4(target.x, target.y, target.z, 1.0f));
    }

    public void ResetMaterialTime()
    {
        DrawMaterial.SetFloat("_StartTime", Time.timeSinceLevelLoad);
    }
}